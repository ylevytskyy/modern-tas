# nCall-Clone — High-Level Architecture (v0.1)

Architecting for a multi-tenant TAS SaaS with **drop-in nCall `/v1` compat**, **HIPAA + GDPR + PCI** compliance, **Kamailio + rtpengine + Asterisk** telephony, and a **6-9 month MVP** with parallel team execution.
...
| P4 | **Two API surfaces, one domain** | `/v1` (nCall-compat XML/JSON) and `/api/v2` (modern JSON) are both first-class facades over the *same* domain services — `/v1` is not a translation shim. |
...
| **M09** | **On-Call Scheduling** | `oncall_schedule` (rrule), `oncall_shift_override` | `/v1/OnCall`, `/api/v2/schedules`, `WhoIsOnCall(account_id, at)` query (FR-S3) | Calendar adapter (sync) | `schedule.updated`, `oncall.gap_detected` |
...
| **M23** | **Calendar Adapter** | Integration backend | Google + Microsoft 365 OAuth2. Read-only MVP. Maps to `oncall_schedule` events. |
...
| **M25** | **REST API Facade (`/v1` + `/api/v2`)** | API-surface backend | XML serializer for `/v1` (verbatim nCall field order — uses `fast-xml-parser` with locked element order + schema fixture tests). Both surfaces import the same domain services. OpenAPI 3.1 emitted from NestJS decorators + Zod schemas. |
...
| **F03** | **`apps/web-console` — Operator Console** | FE: Console pillar | nCall-parity layout (PRD Appendix B). SIP.js wrapper, multi-line state machine, screen-pop subscriber, form runner, save-and-dispatch flow. |
...
Sprint 8-11  ── Scheduling, Supervisor, Portal, Billing ──────────────────
  Domain BE:     M08 Dispatch (incl. escalation), M09 Scheduling
                 (incl. WhoIsOnCall single source), M11 Tasks,
...
- Doesn't decide who's on call — calls `M09.whoIsOnCall(account_id, now)` via the application service contract.
