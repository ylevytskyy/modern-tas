# nCall-Clone — High-Level Architecture (v0.2)
...
| M09 | On-Call Scheduling | unchanged + materialised `oncall_shift` view (closes FR-S5 gap) | Coverage-gap event consumed by M15 (live) + M27 (digest); §4.4 fan-out, no new module |
...
| **M15** | **Supervisor Live State** | `operator_presence` (Redis projection), `coach_session` | **Server is source of truth** for operator state (§4.4 worked decision). Owns `Available`, `OnCall`, `Wrap`, `Offline`; F03 owns intent states `Break`/`Lunch`/`Training`. |
...
| **M25** | **REST API Facade (`/v1` + `/api/v2`)** | **XML output via `xmlbuilder2`** (insertion-order-preserving `.ele()` chain) + **byte-for-byte golden fixtures** captured from the running nCall reference (round-trip diff in CI per endpoint). `fast-xml-parser` reserved for *parsing* inbound XML only. OpenAPI 3.1 emitted from NestJS decorators + Zod schemas. |
...
/contracts/
  ...
  examples/
    *.json
  fixtures/
    v1-xml/                       ← NEW. byte-for-byte captured nCall responses for round-trip diff.
```
