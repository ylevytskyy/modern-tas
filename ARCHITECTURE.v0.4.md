# Telephone Answering Service (TAS) — High-Level Architecture (v0.4)

**Status:** Draft, supersedes [v0.3](./ARCHITECTURE.v0.3.md). Restructures the build into an explicit **Proof of Technology → MVP → Release** progression and absorbs the PRD v2 functional gaps that v0.3 did not number as modules (Queue, Voicemail, IVR, Inbound SMS, Reminders, PHI-scrub, i18n/a11y, TAS-fixture sprint).
...
3. **Six existing modules extended** to close §5.10 (Operator Home), §5.14.1 (PHI scrub), §5.19 Reminders, §5.20 (TZ / i18n / a11y), §11 TAS compatibility-fixture work, and §10 report-definition pinning.
...
- 0.25 product / compliance lead (S7 sales/counsel coordination, S6 TAS fixture capture)
...
The hypotheses + go/no-go signals below are the v0.4 plan **as-designed**. Executed results are back-propagated in the **Result 2026-05-13** column and expanded in §2.5; both are **pending G0 sign-off ratification**.

| # | Spike | Hypothesis | Go/no-go signal | Owner | Result 2026-05-13 |
|---|---|---|---|---|---|
| **S1** | **End-to-end telephony happy path** | Kamailio dispatcher + rtpengine + Asterisk 22.9 LTS + ARI Outbound WS sustains a single registered softphone call through a Kamailio fail-over | New INVITEs route to a healthy Kamailio node within 30 s of primary kill; in-flight call on the failed node drops cleanly (no zombie channels in `GET /channels` after 60 s reconciliation). p95 screen-pop ≤ 800 ms at idle | Telephony eng | **Green (Layer 1 — signalling only)**. screen-pop p95 = 2 ms (≈400× margin); failover TTFOK = 573 ms (≈52× margin); 0 zombie channels after 110/110 calls. Layer 2 (rtpengine media) deferred to Sprint 0 — `network_mode=host` is a no-op on Docker Desktop. See §2.5. |
| **S2** | **NestJS-arbitrated queue dequeue latency** | NestJS holding 200 callers in MOH bridges and dequeueing on operator-accept stays under 200 ms p95 ringing latency (FR-Q10 risk) | SIPp drives 200 callers into a single Queue; NestJS dequeue → ARI `Bridge` → operator-WS `ring` event p95 ≤ 200 ms over a 10-minute window. Failure modes (Redis lock contention, NATS lag) explicitly probed | Telephony + backend eng | **Green**. dequeue p95 = 6 ms (≈33× margin) over 5 539 samples in a 10-min window; Redis + NATS hot-path exercise confirmed. ADR-0024 ratified. Contention probes authored but unrun — baseline margin made them optional. See §2.5. |
| **S3** | **ARI leader 100 ms hard-stop** | The ADR-16 design (close WS within 100 ms of missed Redis heartbeat) is implementable on a real Asterisk 22.9 LTS with @ipcom/asterisk-ari | Chaos test: pause leader process for 5 s. WS observed closed at the Asterisk side within 100 ms of heartbeat miss (verified via Asterisk `WebSocketEvent` log + tcpdump). Replacement leader closes orphaned channels within 7 s | Telephony eng | **Green**. wire close-latency = 1 ms (≈100× margin); reconcile-from-chaos = 1474 ms (≈4.7× margin). ADR-0016 corrected (TTL > HB recommended; second-WS-accepted finding) + ratified. See §2.5. |
| **S4** | **Two-pass redaction accuracy on 8 kHz μ-law** | AssemblyAI Universal-3 Pro Medical + Presidio NER + segment-boundary fallback achieves ≥ 95% recall on planted MRN/DOB/account/phone spans and over-bleeps by ≤ 3 s per false-low-confidence span | 30 synthetic 8 kHz μ-law fixtures with 90 planted PII spans (mix of clean, noisy, accented). Output: recall ≥ 95%, F1 ≥ 0.92, mean over-bleep ≤ 1.5 s, manual-QA backlog ≤ 2% of spans. Fixtures saved to `/contracts/fixtures/redaction-audio/` for X10 | Backend eng + compliance lead | **Deferred-with-fallback-plan** (per §2.4 amendment). AssemblyAI Universal-3 Pro Medical key + 30 annotated 8 kHz μ-law fixtures unavailable; any substitution erases the hazard. ADR-0013 stays Proposed. Adopted outcome: sub-decision deferred 2 weeks post-G0 to a dedicated compliance review (proposal §S4 options A/B/C/D). See §2.5. |
| **S5** | **Supavisor `SET LOCAL` parity** | Supavisor transaction-mode pooling honours `SET LOCAL` boundaries even when the underlying server connection is reused across transactions (the entire RLS-defense-in-depth design depends on this) | ADR-18 test runs Green. Negative case (next transaction on the same pooler connection sees empty `app.tenant_id`) verified. If Red: switch fallback to PgBouncer 1.22+ transaction-mode and adjust ADR-01/18 wording before Sprint 0 closes | SRE | **Green**. Supavisor 1.1.66 transaction-mode honours `SET LOCAL` across transactions on a shared server backend (pid asserted equal across two transactions in one client session). ADR-0018 ratified. See §2.5. |
| **S6** | **`/v1` byte-for-byte fixture capture from live TAS** | A read-only test tenant on a real TAS instance can be cloned into golden fixtures sufficient for M25 to pass round-trip tests | 200 captured XML responses (every consumed resource, every consumed query shape) committed to `/contracts/fixtures/v1-xml/`. Unknown-quirk inventory committed to `docs/tas-compat/quirks.md`. Failure case: vendor blocks our test tenant — fall back to scraping the existing CRM's response cache | Compliance lead + backend eng | **Deferred-with-fallback-plan** (per §2.4 amendment). Live TAS instance + CRM-consumed-endpoint inventory unavailable. No primary ADR. Adopted outcome: Yellow fallback — HAR-export-from-CRM-browser scrape via `tools/crm-har-to-fixtures/scrape.mjs` (commit `f5fb448`) as MVP-baseline; live-TAS-capture is a Sprint-N upgrade. See §2.5. |
| **S7** | **Temporal Cloud BAA + EU namespace metadata egress** | Temporal Cloud Enterprise tier signs the BAA and confirms in writing that EU-namespace Search Attribute metadata does not egress to the US control plane | Sales letter attached to ADR-15. If Red: pivot to self-host Temporal via Helm chart v1.0.0 on EU-residency K8s (already documented as fallback). PoT exit blocked on this answer | Compliance lead | **Deferred-with-fallback-plan** (per §2.4 amendment). 2–6 wk vendor sales/legal cycle blocks Phase 0; nothing synthesisable. **Adopted outcome: parallel Option C** — self-hosted Temporal v1.0.0 Helm on EU-residency K8s as **MVP-baseline** (ADR-0015 §Decision rewritten in commit `13ca47b`); Temporal Cloud Enterprise remains a Sprint-N upgrade path if a BAA letter arrives. See §2.5. |
| **S8** | **Caddy 2.10+ permission + LE rate-limit posture** | The `permission http` endpoint sustains the storage-flood class (certmagic #174) and the LE rate-limit exemption application is in flight | 1 k unknown-SNI probes/sec → permission endpoint declined-LRU absorbs, Caddy storage RPS stays under 50/sec, HAProxy trips before that. LE exemption form submitted (2–4 week ISRG turnaround acceptable since MVP doesn't ship custom domains until Sprint 8) | SRE | **Green (with ADR-0019 amendments)**. HAProxy SNI rate-limit rejected 58 193 / 59 597 connections (97.6 %); permission decisions reduced ~59 k → 813; 0 new cert files in storage in both scenarios. Confirmed no Caddy decline-LRU exists — storage thrash is killed by Caddy's short-circuit on non-2xx ask response. ADR-0019 corrected (3 findings) + ratified. See §2.5. |

> **Amendment 2026-05-13 (pending G0 sign-off ratification)** — the S7 row above documents the planned hypothesis (Cloud BAA as primary, self-host as fallback). The adopted PoT outcome **inverts that commitment**: self-hosted Temporal v1.0.0 Helm on EU-residency K8s is now MVP-baseline; Temporal Cloud Enterprise is a Sprint-N upgrade triggered if a BAA letter arrives. ADR-0015 §Decision rewritten in commit `13ca47b`; ratifies on Sprint-0 self-host operational baseline + SDK identity validation (see §3 G1 below). Pending G0 sign-off ratification alongside the rest of the bundle (`pot/g0-signoff-proposal.md`).
...
### 2.5 PoT outcomes (2026-05-13, pending G0 sign-off ratification)

All eight spikes have been visited in this Phase-0 cycle. **Five hit Green** (S1 Layer 1, S2, S3, S5, S8); **three are Deferred-with-fallback-plan** (S4, S6, S7) per the §2.4 amendment. Full per-spike measurement traces in `pot/pot-readout.md`; per-spike evidence dirs as listed below.

| # | Status | Headline measurement / outcome | ADR(s) updated | Evidence dir |
|---|---|---|---|---|
| **S1** | **Green** (Layer 1 — signalling only) | screen-pop p95 = 2 ms (vs 800 ms — ≈400× margin); failover TTFOK = 573 ms (vs 30 s — ≈52× margin); 0 zombie channels after 110/110 calls. Layer 2 (rtpengine media) deferred to Sprint 0 — `network_mode=host` is a no-op on Docker Desktop, blocking media-path measurement on macOS hosts. | none (S1 has no primary ADR) | `pot/S1-telephony-happy-path/results/20260513T061347Z/` |
| **S2** | **Green** | dequeue p95 = 6 ms (vs 200 ms — ≈33× margin) over 5 539 samples in a 10-min window; Redis lock-renewal + NATS publish on the hot path confirmed (Redis `SET` 118 → 5 983; NATS `in_msgs` 22 → 11 532). Contention probes (Redis netem, NATS netem) authored but unrun — baseline margin made them optional. | **ADR-0024 ratified** Proposed → Accepted (commit `7c6687f`) | `pot/S2-queue-dequeue-latency/results/20260513T042757Z/` |
| **S3** | **Green** | wire close-latency = 1 ms (vs 100 ms — ≈100× margin); reconcile-from-chaos = 1474 ms (vs 7 s — ≈4.7× margin); 20 → 0 channel cleanup confirmed. | **ADR-0016 corrected + ratified** (commits `18ae71f` + `8d0bdf3`). Two amendments: **(a)** TTL > HB recommended (3:1 ratio, e.g. 500 ms HB + 1500 ms TTL) — heartbeat-at-TTL-boundary causes both leaders to flap on the renew `GET`; **(b)** Asterisk *accepts* a second WS for the same Stasis app on 20.6 — orphan window is bounded by lease TTL alone, not lease TTL + close-latency + new-WS handshake (tighter than the original Decision claimed). | `pot/S3-ari-leader-hard-stop/results/20260513T052041Z/` |
| **S4** | **Deferred-with-fallback-plan** | Phase 0 not attempted. AssemblyAI Universal-3 Pro Medical key + 30 annotated 8 kHz μ-law fixtures unavailable; any substitution (Whisper, TTS-downsample) erases the ADR-0013 hazard (medical-domain ASR boundary accuracy under telephony band-limiting). | ADR-0013 stays Proposed. **Adopted Deferred outcome (pending ratification):** sub-decision deferred 2 weeks post-G0 to a dedicated compliance review session (proposal §S4 options A/B/C/D). Recording / redaction MVP modules pause for the sub-decision window (acceptable — they ship mid-MVP, not at start). | n/a (Phase 0 not attempted) |
| **S5** | **Green** | Supavisor 1.1.66 transaction-mode honours `SET LOCAL` boundaries across transactions on a reused server backend. Two transactions in one psql session (`pool_size=1`) both ran on backend pid 134; transaction 2 read empty `app.tenant_id` after transaction 1's `SET LOCAL` committed. RLS-defense-in-depth precondition validated. | **ADR-0018 ratified** Proposed → Accepted (commit `6a1507c`) | `pot/S5-supavisor-set-local/results/20260513T033050Z/` |
| **S6** | **Deferred-with-fallback-plan** | Phase 0 not attempted. Live TAS instance access (vendor sandbox or read-only test user) + CRM-consumed-endpoint inventory both unavailable; any synthesis (mock server, PRD §7.5-driven generator) erases the unknown-deviation hazard. | none (S6 has no primary ADR — feeds M25 module spec directly). **Adopted Deferred outcome (pending ratification):** Yellow fallback — HAR-export-from-CRM-browser scrape via `tools/crm-har-to-fixtures/scrape.mjs` (commit `f5fb448`, 20 tests pass + smoke-tested) as MVP-baseline; live-TAS-capture is a Sprint-N upgrade triggered if vendor access lands, M25 round-trip tests start failing in production, or feature work needs an endpoint the HAR doesn't cover. | n/a (Phase 0 not attempted) |
| **S7** | **Deferred-with-fallback-plan** | Phase 0 not attempted. Temporal Cloud sales/legal correspondence is a 2–6 wk calendar cycle; not initiated; the verdict is the letter itself, so nothing is synthesisable. | **ADR-0015 §Decision rewritten** (commit `13ca47b`, pending G0 ratification) — swaps primary/fallback. **Adopted Deferred outcome (pending ratification): parallel Option C** — self-hosted Temporal v1.0.0 Helm on EU-residency K8s as MVP-baseline AND initiate Cloud sales correspondence so Temporal Cloud Enterprise remains a Sprint-N upgrade if a BAA letter arrives. Cost: +0.5–1 FTE platform-engineering for cluster operation. ADR-0015 ratifies on Sprint-0 SDK identity validation + self-host operational baseline. | n/a (Phase 0 not attempted) |
| **S8** | **Green** (with ADR-0019 amendments) | **Scenario A** (k6 → HAProxy → Caddy): HAProxy TCP SNI rate-limit rejected 58 193 / 59 597 connections (97.6 %); permission decisions reduced ~59 k → 813. **Scenario B** (k6 → Caddy direct): 59 243 permission decisions for 51 distinct SNIs confirmed *no* Caddy decline-LRU exists. **Storage delta = 0 new cert files in both scenarios** — storage thrash hazard killed by Caddy's short-circuit on non-2xx ask response. | **ADR-0019 corrected + ratified** (commits `8dd7643` + `03b3a9d`). Three amendments: **(a)** Decision §1 "Caddy LRU-caches declined SNIs" claim removed — actual mechanism is Caddy short-circuits storage I/O on non-2xx ask response (declined SNIs never reach `LoadCertificate`); **(b)** defence-in-depth layer (b) re-attributed from Caddy to the permission endpoint — it is now the load-bearing rate-limit surface and needs its own Redis-backed rate-limiter sketched in Consequences; **(c)** threshold-tunability footnote added — PoT validated at 800/s/source (macOS Docker k6 cap); production stays at 1000/s/source pending a Sprint-0 Linux re-test. | `pot/S8-caddy-le-posture/results/20260513T093513Z/` |

**ADR ratification summary**

| ADR | State 2026-05-13 | Source spike |
|---|---|---|
| ADR-0013 (PCI/HIPAA redaction) | Stays Proposed; close-out gated on Sprint-0 compliance review (S4 sub-decision) | S4 |
| ADR-0015 (Temporal BAA tier) | §Decision rewritten (commit `13ca47b`) — self-host MVP-baseline, Cloud Sprint-N upgrade; ratifies on Sprint-0 self-host operational baseline + SDK identity validation. **Pending G0 sign-off ratification.** | S7 |
| ADR-0016 (ARI leader hard-stop) | Corrected (commit `18ae71f`) + ratified Proposed → Accepted (commit `8d0bdf3`) | S3 |
| ADR-0018 (Supavisor SET LOCAL) | Ratified Proposed → Accepted (commit `6a1507c`) | S5 |
| ADR-0019 (Caddy permission + LE) | Corrected (commit `8dd7643`) + ratified Proposed → Accepted (commit `03b3a9d`) | S8 |
| ADR-0024 (Queue dequeue latency budget) | Ratified Proposed → Accepted (commit `7c6687f`) | S2 |

**Status of this section** — pending G0 sign-off ratification, alongside the §2.4 amendment, the §3 G1 swap below, the ADR-0015 §Decision rewrite (commit `13ca47b`), the per-spike forensic notes in `pot/pot-readout.md` (commit `1109af1`), and the Sprint-0 carry-over scaffold `tools/crm-har-to-fixtures/scrape.mjs` (commit `f5fb448`). See `pot/g0-signoff-proposal.md` for the bundle, revert sequence, and per-Deferred sub-decision rationale.
...
- **Sprint-0 deliverables overlap with PoT weeks 2–4**: EU counsel review of ADR-14 template; **self-host Temporal operational baseline + SDK identity validation for ADR-15** (per the 2026-05-13 §Decision rewrite, see §2.5 — Temporal Cloud sales correspondence continues in parallel as a Sprint-N upgrade trigger); LE rate-limit exemption submission for ADR-19. These are the long-lead external-dependency items.

**Exit gate G1**

- 30 ADRs merged in `docs/adr/`.
- EU counsel written opinion on ADR-14 attached as `docs/adr/0014-eu-counsel-opinion.pdf`.
- **Self-host Temporal operational baseline + SDK identity validation evidence attached to ADR-15** (per the 2026-05-13 §Decision rewrite, commit `13ca47b`); Temporal Cloud BAA letter, if pursued in parallel and received, attached as `docs/adr/0015-temporal-baa-tier.pdf` to ratify a Sprint-N Cloud-upgrade path rather than the MVP baseline.
- LE exemption application receipt in `docs/adr/0019-le-exemption-receipt.pdf`.
- G0 also signed off (PoT exit).

> **Amendment 2026-05-13 (pending G0 sign-off ratification)** — the bullet and gate item above swap ADR-15's MVP commitment from "Temporal Cloud Enterprise BAA letter" to "self-host Temporal v1.0.0 Helm on EU-residency K8s operational baseline + SDK identity validation". Triggered by S7's Deferred-with-fallback-plan outcome (§2.5) and the ADR-0015 §Decision rewrite at commit `13ca47b`. Cloud BAA correspondence continues in parallel; if a letter arrives, it ratifies a Sprint-N upgrade path, not the MVP baseline. Reverts cleanly if Path A (Strict) is chosen at G0 sign-off — see `pot/g0-signoff-proposal.md`.
...
─── Sprint 8–11 (8 weeks) ── Scheduling, Supervisor, Portal, Billing, ──────
  ...
  v0.3 Sprint 8–11 scope, PLUS:
   ...
   • TAS compatibility sprint (S6 fixtures → M25 round-trip tests + quirk
...
─── Sprint 12–15 (8 weeks) ── Hardening + Compliance proof ─────────────────
  v0.3 Sprint 12–15 scope unchanged, PLUS:
   ...
   • Pilot tenant onboarding rehearsal — full migration-assistance bundle
     dry-run against a real TAS export via M28.
...
| TAS test-tenant access denied → S6 fixtures absent | Medium | Medium | Fall back to scraping existing CRM response cache; if also denied, raise to product as a P1 — MVP CRM-compat smoke (AC §12 #6) cannot ship without fixtures. |
...
2. **Four new MVP-scope modules (M30 Queue, M31 Voicemail, M32 IVR, M33 Inbound SMS)** plus five extensions (Reminders in M11, PHI-scrub + POST-search in M25, i18n+a11y in F01, Operator Home in F03, TAS fixture work in M28/M25). AC §12 #15–#20 now have explicit owners and sprint placement.
