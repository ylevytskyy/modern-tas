...
The design spec exit criterion says `drizzle-kit migrate` exits 0, and uses the invocation style `pnpm --filter @ncall/db migrate` throughout. Renaming the npm script from `migrate:apply` to `migrate` makes the spec-mandated invocation work without needing `pnpm exec drizzle-kit migrate --config ...` wrappers. It is also cleaner. This name change appears in Task 2 (package scaffold) and all subsequent task steps that call `migrate`. The migration generation script remains `migrate:gen`.
...
| `packages/db/package.json` | Create | @ncall/db manifest; scripts: `migrate:gen`, `migrate`, `seed`, `typecheck`, `test` |
...
| `infra/postgres/init.sql` | Create | Creates `_supavisor` DB + schema for Supavisor meta-db; creates `ncall` app user with schema GRANT |
...
| `infra/asterisk/extensions.conf` | Create | DID +15555550100 → Stasis(ncall) |
| `infra/asterisk/pjsip.conf` | Create | UDP transport + kamailio endpoint |
| `infra/asterisk/ari.conf` | Create | ARI user `ncall`/`ncall` |
...
```bash
pnpm install
pnpm --filter @ncall/db typecheck
```
...
```bash
docker run --rm -d --name ncall-test-pg \
  -e POSTGRES_USER=ncall -e POSTGRES_PASSWORD=ncall -e POSTGRES_DB=ncall_test \
  -p 5433:5432 postgres:15
# Wait for postgres to be ready
until docker exec ncall-test-pg pg_isready -U ncall; do sleep 1; done
DATABASE_URL=postgres://ncall:ncall@localhost:5433/ncall_test pnpm --filter @ncall/db migrate
...
No TDD — deterministic seed. Verification = `pnpm --filter @ncall/db seed` against a fresh DB; psql confirms 1 DID row.
...
```bash
docker run --rm -d --name ncall-seed-pg \
  -e POSTGRES_USER=ncall -e POSTGRES_PASSWORD=ncall -e POSTGRES_DB=ncall \
  -p 5434:5432 postgres:15
until docker exec ncall-seed-pg pg_isready -U ncall; do sleep 1; done
DATABASE_URL=postgres://ncall:ncall@localhost:5434/ncall pnpm --filter @ncall/db migrate
...
5. App connections use `ncall.ncall` (tenant-suffixed username, tenant ID = `ncall`) on port 6543.
...
> **NC2 fix applied here:** Postgres 15 changed default-grant behaviour — `GRANT ALL PRIVILEGES ON DATABASE` does NOT convey CREATE on the `public` schema. The explicit `GRANT ALL ON SCHEMA public TO ncall;` inside the `\connect ncall` block is required; without it `drizzle-kit migrate` fails with "permission denied for schema public". Logical ordering: create `_supavisor` DB → set up `_supavisor` schema → `\connect ncall` (ncall DB already exists per `POSTGRES_DB: ncall`) → create `ncall` user → grant DB privileges → **grant schema privileges**.
...
```yaml
# ncall-clone PoC stack — services grow through Tasks 8–15.
...
services:
  ...
  supavisor-migrate:
    ...
    environment:
      DATABASE_URL: postgres://postgres:ncall@postgres:5432/_supavisor
...
**Important note on Postgres user:** We use `POSTGRES_USER: postgres` (the superuser, same as S5) so that Supavisor's `DATABASE_URL` can connect to the `_supavisor` meta-database. The application database `ncall` is created as `POSTGRES_DB: ncall`. The application user `ncall` with password `ncall` is created in the init.sql `\connect ncall` block (Step 7.1). The Drizzle schema uses `ncall` as both username and password for the **application** connection; the superuser `postgres` is only used by Supavisor internally.
...
# Register the 'ncall' tenant via admin API.
...
# Smoke: psql through Supavisor with tenant-suffixed username ncall.ncall
docker compose -f infra/docker-compose.yml exec postgres \
  psql "postgres://ncall.ncall:ncall@supavisor:6543/ncall" -c "select 1;"
...
# ncall PoC — Kamailio single-node proxy.
...
docker compose -f infra/docker-compose.yml exec asterisk \
  curl -s -u ncall:ncall http://localhost:8088/ari/asterisk/info | head -c 200
docker compose -f infra/docker-compose.yml down
```
...
**Note on Temporal Postgres user:** Temporal connects to Postgres as `postgres` with password `ncall`. This is the superuser we defined for the compose Postgres service.
...
```bash
# psql through Supavisor — note tenant-suffixed username: ncall.ncall
...
- [ ] **Step 17.8: pnpm --filter @ncall/db run seed connects to compose Postgres from host**
...
- [ ] Did `pnpm --filter @ncall/db test` go RED (import error) before schema was written, then GREEN after? Confirm for each of Tasks 3, 4, 5 individually.
...
- [ ] Does `make poc-seed` use `DATABASE_URL=postgres://ncall:ncall@localhost:5432/ncall` (direct Postgres, not Supavisor port 6543)? Drizzle-kit migrate requires a direct non-pooled connection.
...
- [ ] Did `make poc-up` successfully register the Supavisor tenant `ncall`? Confirm by checking that `_supavisor-register-tenant` printed "Supavisor tenant 'ncall' registered".
- [ ] Did the Supavisor smoke test use the tenant-suffixed username `ncall.ncall` (not bare `ncall`)? Bare `ncall` will be rejected by Supavisor when `require_user: true`.
...
- [ ] Does the `infra/postgres/init.sql` contain `GRANT ALL ON SCHEMA public TO ncall;` in the `\connect ncall` block? Without it, `drizzle-kit migrate` fails on Postgres 15 with "permission denied for schema public".
...
5. The `POSTGRES_DB: ncall` docker-compose env causes Docker's entrypoint to create the `ncall` database before `docker-entrypoint-initdb.d/` scripts run, making `\connect ncall` in `init.sql` succeed.
