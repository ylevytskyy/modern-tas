...
- **S9 (Asterisk-direct audio loopback) — Green on Linux at `b6139c9` (2026-05-16).** Confirms ADR-0025 carries two-way ulaw audio on bench scale through Docker Desktop's UDP NAT, fully replacing the S1 Layer-2 rtpengine media smoke as the audio-path evidence for the MVP edge topology. The S1 Layer-2 trigger ("Green before Chunk 3 commit 1") is satisfied by S9 plus the existing Chunk 3 Green at `a7c2dac`, and the S1-Layer-2 Pending status is reclassified as **Resolved-via-S9** for MVP purposes. The Kamailio-fronted-SBC carry-over (re-introduce S1 Layer-2 if the SBC tier returns) remains. Evidence: `pot/S9-audio-loopback/results/2026-05-16T07-48-47Z-loopback.md`, tag `pot/S9`.
